...
## 二、当前进行中
1. **M1「能做品类」边界已落地 ✅**（v0.73，P-031）：白名单 9 类为准（家居日用/厨房用品/宠物用品/收纳整理/个护清洁/服饰配件/数码配件/办公文具/户外运动）；食品/饮品永久排除；商品池纯白名单品（重跑候选 1：锅刷）。**当前抖店商品榜食品占比高 → 白名单内可做品少，需有米云重登补采 + 白名单类目定向选榜（后续排期）**。
2. 服务运行中：API(8001)+前端(3000)+共享Chrome(9223)+有米云(9555)。
3. 待办：**有米云 9555 重登后补采第三源（品类更多）**、M5 v1.1 半自动实投（¥50 级，用户批准后）、M4 真实草稿（旧系统 UI 函数迁移）。
...
- **P-031（用户裁定「只找白名单里的品，其他的不要找」+「9 类为准」）：能做品类边界落地——category_map 类目解析器 + permanent_exclusion（127 词）接入 hard_reject + 白名单强制（类目空/白名单外 → hard_reject）；词表修正（移除黄金/姜误伤、补 76 食品词、safe 豁免 27 词）；真实重跑候选 1（锅刷）拒 107，商品池纯白名单品（203 passed）。**
