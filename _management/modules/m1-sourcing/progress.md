...
## P-035/P-036 修复后全量询价收官（2026-09-01 · 66 商品 65 个有成本）

- **P-035（用户观察驱动）**：共享浏览器 18 标签中 16 个询价残留 detail 页 → connect 初始化扫描挂起（P-029 卡死主因确认）→ 询价调度**每块前 zombie-clean**，块超时从「每块必超时」降为「无超时」。
- **P-036**：umcdn 图 auth_key 时效过期（隔夜 403）→ 搜图全失败 → 重采新签名图 + 立即询价；根治=采集时图片本地化落盘（排期）。
- **加速**：max_suppliers=3 → 1（每商品 130s → 5-8s），66 商品 5 分钟完成。
- **最终结果**：pool 66 → **65 个有成本（98.5%）**，平均毛利 75.5%；类目全覆盖（个护 21/家居 15/文具 9/厨房 8/配件 7/户外 2/宠物 2/收纳 1）；成本/售价/毛利/供应商全部回写。证据 `_management/logs/m1_quote_results_final_20260901.jsonl`。
- **登记**：pitfall-log P-035/P-036、dashboard。

## 多供应商补询实测（2026-09-01 · P-037）

- 用户要求补询多供应商 → max_suppliers=3 全量补询 65 商品 → **quotes 分布全部为 1**（1688 以图搜款精确匹配通常仅 1 家；「找相似」卡片读价链路多卡失败）。
- **结论**：多供应商瓶颈在卡片处理（广告位无 offerId/部分 detail 价格区未命中），非参数；接受「每商品 1 家供应商」为当前现实状态（商品池 65 品成本齐全可直接上架）。
- **排期**：找相似卡片读价修复/同款聚合（多供应商）；按标题站内搜同款需用户裁定（P-027 曾废弃标题搜索）。
- **登记**：pitfall-log P-037。

## P-040 白名单类目定向采集（2026-09-01 · 用户提议「罗盘可筛品类」）

- **实现**：config `doudian_categories`（8 类目标：个护家清/智能家居/餐饮厨具/宠物生活/服饰内衣/3C数码/运动户外/办公设备，industry_id/category_id 为罗盘 API 参数）；doudian `collect_category`（导航→cascader 选行业+二级/全部→读表）+ `_wait_table_rows` 轮询；CLI `collect-categories`。
- **首次验证**：个护 110 / 家居 108 / 厨具 107 / 宠物 10 = **335 条真实白名单商品**（带图带价，来源=罗盘 TOP200 类目榜）。
- **限流**：探测+重复采集频率过高触发罗盘接口限流（切换类目表格卡 loading）→ 冷却后低频重跑（类目间隔 10-20s、单轮 ≤4 类）；登记 P-040。
- **登记**：pitfall-log P-040、dashboard。
