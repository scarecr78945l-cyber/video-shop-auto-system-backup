...
| [ ] P5 Playwright 兜底降级+集成（UI 兜底通道、page_changed、与 M1/M3 数据契约联调、端到端模拟）**可拆子代理** | 子代理 P5（派发中） | 5%（派发中） | 兜底通道→流水线编排→端到端模拟→验收 | v1.2 |
...
- 本模块当前完成度：**75%**（P1/P2/P3/P4 均验收通过；P5 兜底降级+集成派发中）
- 已完成：筹备 P0 全部；P1 OpenAPI 薄封装（6 测试）；P2 上架校验硬门禁（25 测试）；P3 状态机与证据（31 测试，R22 断言固化）；P4 拒审处理（36 测试）；P1 文档核对（context/external-contracts.md）
- 剩余：P5 兜底集成（UI 兜底通道 + 流水线编排 + 端到端模拟）→ P6 M5 衔接验收
- 排期建议：P5（依赖 P1–P4 已满足）→ P6（依赖 P5 + M5 就绪）
