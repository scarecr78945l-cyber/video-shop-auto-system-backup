...
## B2/B3 实机探测确认（2026-08-30 ｜ 总控，CDP 9223 只读）

- **投放 dashboard（/shop/promotion/#/dashboard）结构已实机确认**（与 08 文档后台事实锚点完全吻合）：
  - 双入口：商品托管 / 直播投放；
  - 托管状态：已托管 / 待托管（计数区）；
  - 投放概览：花费 / 曝光 / 成交订单数 / 成交金额 / 实际成交ROI；
  - 智能诊断（"1项提升建议"）；「添加待托管商品」（提示"再添加7个待托管商品"）；
  - 本月投放：16 条计划 / 20 天（账号有真实投放历史）；
  - 账户余额（可用余额区）；素材榜单 / 投放资质 / 素材库 / 类目准入规则入口。
- **B2 就绪**（账号可登录 + 有余额区 + 真实投放历史）；**B3 就绪**（素材库入口存在）。
- **v1.1 真实实投前置条件已齐**：登录态 ✅ + 投放结构确认 ✅；待总控批准小额实投方案（预算三重硬约束保护）。
- 注意：余额具体数值/财务数据不自动化读取（避免风控），由人工在页面确认。