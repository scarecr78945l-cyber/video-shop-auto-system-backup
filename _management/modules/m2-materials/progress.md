...
| [ ] 视频号采集器（自研签名+直链） | 子代理 B1 | 已派发 | 执行中（子代理 id f331c395）；signer.py 接口化可替换（R-M2-03），fixtures 模式 |
| [ ] 淘宝/1688 商品视频与同款图采集 | 子代理 B2 | 已派发 | 执行中（子代理 id 03904cb8）；⚠️fetch_* 半成品不在当前工作区（已勘察）→ 按设计从零实现+fixtures，待迁移包对照 |
| [ ] 考古加/有米云榜单图缓存（IMAGE_CACHE） | 子代理 B3 | 已派发 | 执行中（子代理 id f833480a）；⚠️考古加未实现（REC-006 降级）→ 先做有米云，多源接口预留 |
...
- 本模块当前完成度：**45%**（筹备 15% + 批次 1 15% + 批次 2 15%：素材库表/下载中台/双去重/标准化器/TikTokDownloader 封装全部验收）
- 里程碑达成：`asset_* 表可建` ✅、`下载中台可跑` ✅、`双去重可用` ✅、`ffmpeg 标准化器（mock 模式）` ✅、`TikTokDownloader 封装（fixtures）` ✅
- 环境待确认：**ffmpeg 未安装**（标准化器 mock 模式，就绪后自动切真实 runner）、**TikTokDownloader 未安装**（封装就绪，就绪后装 4.1.x 二进制）
- 距离 v1.0 还差：批次 3（B 视频号采集器、淘宝1688 复测、榜单图缓存）→ 批次 4（标签合规、M3/M5 联调）→ 集成验收
