...
| [ ] 双去重器（视频 MD5+关键帧 phash / 图片 phash） | 子代理 E | 已派发 | 执行中（子代理 id 4179c644） |
| [ ] ffmpeg 标准化器（硬规格校验+转码+元数据） | 子代理 C | 已派发 | 执行中（子代理 id 487ca61b）；**环境待确认：ffmpeg 未安装（已探测）→ mock 模式实现，环境就绪切换** |
| [ ] TikTokDownloader 二次封装（抖音/快手/小红书） | 子代理 A | 已派发 | 执行中（子代理 id 475a06d1）；**环境待确认：TikTokDownloader 未安装（已探测）→ 锁定版本设计封装+fixtures 测试** |
