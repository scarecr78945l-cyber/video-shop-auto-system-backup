...
- **出现时间**：2025 体系建立日 ｜ **模块**：M2 及全局（多模块并行开发） ｜ **代理**：子代理 C（M2 批次 2）
...
- **解决方案**：跑测试前确认无其他代理正在跑 pytest；遇间歇性 errors 先串行复跑确认；必要时各模块隔离 basetemp（如 `.pytest-tmp-<模块>`）。
- **防复发**：① 总工验收测试结果以「连续两次全绿」为准；② 子代理任务书注明「间歇性 errors 先串行复跑排除环境干扰」；③ 全量测试尽量错峰执行。
