"""M5 自动小店投放（商品托管）模块 · SQLAlchemy 2.0 引擎/会话管理。

默认 SQLite（本模块独立库 data/db/m5-ads.db），生产通过 ADS_DB_URL
切 PostgreSQL。一模块一库铁律：只操作本模块库。
...
class Database:
    ...
    def __init__(self, config: AdsConfig):
        ...
        if config.db_url.startswith("sqlite"):
            ...
            if config.db_url.startswith("sqlite:///"):
                # 文件型 SQLite：确保父目录存在（默认库 data/db/m5-ads.db，目录可能尚未创建）
...
def default_database(config: Optional[AdsConfig] = None) -> Database:
    if config is None:
