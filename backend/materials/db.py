"""M2 自动收集素材模块 · SQLAlchemy 2.0 引擎/会话管理。

默认 SQLite（本模块独立库 data/db/m2-materials.db），生产通过
MATERIALS_DB_URL 切 PostgreSQL。一模块一库铁律：只操作本模块库。
...
def default_database(config: Optional[MaterialsConfig] = None) -> Database:
    if config is None:
